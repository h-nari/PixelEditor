...
export class Menu {
  ...
  expand(e: JQuery.ClickEvent, aside: boolean = false) {
    if (this.subMenu.length > 0) {
      ...
      let menu_elem = $(e.currentTarget);
      let offset = menu_elem.offset();
      let h = menu_elem.height() || 0;
      let x = offset?.left || 0;
      let y = (offset?.top || 0) + h + 5;
      if (aside) {
        let off = menu_elem.offset();
        if (off) {
          x = off.left + (menu_elem.width() || 0) + 20;
          y = off.top;
        }
      }
